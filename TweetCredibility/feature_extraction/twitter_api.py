...
if __name__ == "__main__":
    ...
    #!-------------------
    #connection = api_v1_connection()
    client = tweepy.Client(bearer_token="")
    query=[]
    query.append("#covid lang:en")
    query.append("#war lang:en")
    query.append("#ballodor lang:en")

    for el in query:
        tweets = client.search_recent_tweets(query=el, max_results=10)

        with open("miao.txt","a") as file:
            
            file.write(str(tweets.data))
            #print(str(tweets.data))
        # for tweet in tweets.data:
        #     print(tweet.text)
        #     if len(tweet.context_annotations) > 0:
        #         print(tweet.context_annotations)
    #!-------------------
    #*QUESTO PUO SERVIRE SE DOBBIAMO SALVARE IL TWEET APPENA PRESO IN FORMATO JSON
    # import json
